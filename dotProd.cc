...
#else 
    LoopHistory* lhinit = new LoopHistory(1.0); // Use static scheduling for initializing array.
    CkLoop_ParallelizeHybrid(lhinit, initVectors, 2, params, numberOfChunks, 0, (probSize/numElemsPerNode), 1);
#endif
  }
...
#pragma omp for schedule(dynamic,chunkSize) reduction(+:r) // The clause reduction is a hint to LLVM OpenMP to optimize the code for reduct\ion operation.
    ...
    CkLoop_ParallelizeHybrid(lh, dotP_chunked, 2, params, numberOfChunks, 0, (probSize/numElemsPerNode), 1, &r, CKLOOP_FLOAT_SUM);
