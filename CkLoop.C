...
void FuncCkLoop::parallelizeFuncHybrid(float staticFraction, HelperFn func, int paramNum, void * param,
    ...
    TRACE_START(CKLOOP_TOTAL_WORK_EVENTID);
    /*
   printf("debug [%d]:, CkMyRank=%d: funchybrid. \n", CkMyPe(), CkMyRank());
    FuncSingleHelper *thisHelper = helperPtr[CkMyRank()];
#if ALLOW_MULTIPLE_UNSYNC
    printf("debug: %d: funchybrid. 1b thisHelper: %d \n", CkMyPe(), (long) thisHelper);
        ConverseNotifyMsg *notifyMsg = thisHelper->getNotifyMsg();
  printf("ptr %d\n", (long) notifyMsg->ptr);
#else
    printf("debug: %d: funchybrid. 1c \n", CkMyPe());
        ConverseNotifyMsg *notifyMsg = thisHelper->notifyMsg;
#endif
    printf("debug: %d: funchybrid. 2 \n", CkMyPe());
        curLoop = (CurLoopInfo *)(notifyMsg->ptr);
    */

    //curLoop = new CurLoopInfo(FuncCkLoop::MAX_CHUNKS);

    curLoop = new CurLoopInfo(numHelpers);
    curLoop->set(numChunks, func, lowerRange, upperRange, paramNum, param);
  ...
  if(type == CKLOOP_INT_SUM)
    ...
    for(int i=0; i<numHelpers; i++)
      *((float*)redBufs[i]) = 0.0;
  LoopChunkMsg* msg = (LoopChunkMsg *)malloc(sizeof(LoopChunkMsg));
  msg->loopRec = curLoop;
  ...
  // printf("DEBUG: Exiting loop : numChunks = %d \t numStaticChunksCompleted = %d \t numDynamicChunksFired = %d \t numDynamicChunksCompleted = %d \t \n", numChunks, curLoop->numStaticChunksCompleted, curLoop->numDynamicChunksFired, curLoop->numDynamicChunksCompleted);
 ...
 if (PAPI_stop_counters(values, num_hwcntrs) != PAPI_OK)   printf("Error with stopping counters!\n");
    ...
    printf("Total Prof: L2 data cache misses, \t%lld\n", values[0]); // Move this elsewhere to add to global variable.
    printf("Total Prof: L3 data cache misses, \t%lld\n", values[1]);
#endif
...
void CurLoopInfo::doWorkForMyPe() {
  ...
  //printf("DEBUG: chunk [%d:\t %d] function returned %f for reduction\n" , myStaticBegin, myDynamicBegin, x);
